...
static unsigned long base = 0x278;
...
{
	return do_short_read(filp->f_inode, filp, buf, count, f_pos);
}
...
ssize_t short_write(struct file *filp, const char __user *buf, size_t count,
		loff_t *f_pos)
{
	return do_short_write(filp->f_inode, filp, buf, count, f_pos);
}
