...
static int get_size(struct ddk_device *dev)
{
	...
	buf=kmalloc(GFP_KERNEL, sizeof(short));
	/* Control IN */
...
static int get_off(struct ddk_device *dev, int dir)
{
	...
	buf=kmalloc(GFP_KERNEL, sizeof(char));

	/* Control IN */
	retval = usb_control_msg(dev->device, usb_rcvctrlpipe(dev->device, 0),
			(dir == e_read) ? CUSTOM_RQ_GET_MEM_RD_OFFSET : CUSTOM_RQ_GET_MEM_WR_OFFSET,
			USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE,
			0, 0, buf, sizeof(char), 0);
	printk(KERN_ALERT "Write offset %d", retval);
	if (retval < 0)
	{
		return retval;
	}
...
static ssize_t ddk_write(struct file *f, const char __user *buf, size_t cnt, loff_t *off)
{
	...
	if ((retval = set_off(dev, e_write, (write_off))) < 0)
	{
		return retval;
	}
	wrote_cnt = 0;
...
static ssize_t ddk_read(struct file *f, char __user *buf, size_t cnt, loff_t *off)
{
	...
	else
	{
		...
		if (((retval = set_off(dev, e_read, (read_off)))) < 0)
		{
			return retval;
		}
...
#endif
{
	...
	{
		case DDK_LED_GET:
			printk(KERN_ALERT "Inside ioctl LED_GET\n");
			/* Control IN */
			retval = usb_control_msg(dev->device, usb_rcvctrlpipe(dev->device, 0),
		...
		case DDK_RD_OFF_GET:
			...
			printk(KERN_ALERT "get read offset:%d", retval);
			if (retval < 0)
			{
				printk(KERN_ERR "Control message returned %d\n", retval);
				return retval;
