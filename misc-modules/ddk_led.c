...
/* Exported LED control function for use by ddk_mem driver */
int ddk_led_ctrl(int val)
{
    return usb_control_msg(ddk_dev.device,
                usb_sndctrlpipe(ddk_dev.device, 0),
                CUSTOM_RQ_SET_LED_STATUS,
                USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE,
                val, 0, NULL, 0, 0);
}
EXPORT_SYMBOL(ddk_led_ctrl);
